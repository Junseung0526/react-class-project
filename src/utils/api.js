...
export const fetchNewsData = async (query) => {
    const url = `/naver-news-proxy?query=${encodeURIComponent(query)}`;
...
export const summarizeAndTag = async (articleContent, persona) => {
    if (!GEMINI_API_KEY) {
        console.error("Gemini API key is not set in environment variables.");
        return { summary: "AI 요약을 불러올 수 없습니다.", tags: [] };
    }
    ...
    try {
        ...
        console.error("Failed on article content:", articleContent);
        console.error("Gemini API Error details:", error.message); 
        return { summary: "AI 요약을 불러올 수 없습니다. (API 실패)", tags: [] };
    }
};